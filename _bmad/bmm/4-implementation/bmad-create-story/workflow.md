...
<step n="4b" goal="UI design assessment for stories with frontend work">
  <action>Analyze the story's acceptance criteria and tasks to determine if this story involves frontend UI work:
    - New pages or screens
    - New UI components
    - Significant visual changes to existing pages
    - Layout modifications
  </action>

  <check if="story has NO frontend UI requirements">
    <action>Skip to step 5 — no UI design assessment needed</action>
  </check>

  <check if="story HAS frontend UI requirements">
    <action>Check if DESIGN.md exists at project root</action>

    <check if="DESIGN.md does NOT exist">
      <output>**UI Design Foundation Required**

        This story has frontend UI requirements but no design system has been established.
        Run `/stitch-design-foundation` first to create the visual design system and core layouts.
        This ensures all UI work follows a consistent design language.

        **Options:**
        1. Run `/stitch-design-foundation` now (recommended for first story with UI)
        2. Skip design generation and proceed with story (developer will handle UI manually)
        3. Halt and run foundation design separately
      </output>
      <ask>Choose option [1], [2], or [3]:</ask>

      <check if="user chooses '2'">
        <action>Add note to story: "UI design foundation not established — developer should follow UX spec for visual guidance"</action>
        <action>Skip to step 5</action>
      </check>
      <check if="user chooses '1' or '3'">
        <action>HALT — user should run `/stitch-design-foundation` before continuing</action>
      </check>
    </check>

    <check if="DESIGN.md EXISTS">
      <action>Load DESIGN.md and stitch.json for design system context</action>
      <action>Assess what UI elements this story needs:
        - Can existing approved layouts/components from the foundation cover this? → Note reusable assets
        - Does this story need NEW screens or components beyond the foundation? → Flag for Tier 2 design
      </action>

      <check if="existing components cover the story UI needs">
        <action>Add to story file: "UI Design Assets" section listing existing components to reuse, with file paths</action>
        <action>Skip to step 5</action>
      </check>

      <check if="new UI design is needed">
        <action>Run the `stitch-ui-story-design` workflow (Tier 2):
          1. Generate 3 design directions within the established design system
          2. User selects preferred direction
          3. Refine and validate (consistency + accessibility checks)
          4. Convert to Vue components
          5. Cleanup rejected screens
        </action>
        <action>Add generated component file paths and design references to the story's "UI Design Assets" section</action>
      </check>
    </check>
  </check>
</step>
