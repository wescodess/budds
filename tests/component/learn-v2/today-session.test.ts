...
describe('LearnV2TodaySession', () => {
  beforeEach(() => {
    ...
    submit.mockReset().mockResolvedValue({ status: 'completed', scorePercent: 80, state: 'independent' })
  ...
  it('progressively discloses the canonical phase order after start', async () => {
    ...
    for (const phase of ['retrieval', 'objective', 'cold_attempt', 'explanation', 'independent_application', 'confidence_teach_back']) {
      expect(wrapper.find(`[data-testid="learn-v2-phase-${phase}"]`).exists()).toBe(true)
      if (phase === 'cold_attempt') await wrapper.find('textarea[aria-label="Your prediction"]').setValue('prediction')
      if (phase !== 'confidence_teach_back') await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click')
    }
  })
  it('requires prediction, response, and confidence before progressing or submitting', async () => {
    const wrapper = await mount(); await startSession(wrapper)
    await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click')
    expect((wrapper.find('[data-testid="learn-v2-continue"]').element as HTMLButtonElement).disabled).toBe(true)
    await wrapper.find('textarea[aria-label="Your prediction"]').setValue('guess'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click')
  ...
  it('obtains reveal and hint from the server and uses returned revisions', async () => {
    const wrapper = await mount(); await startSession(wrapper)
    await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Your prediction"]').setValue('guess'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click')
  ...
  it('renders pending scoring and retries with the same request key', async () => {
    ...
    const wrapper = await mount(); await startSession(wrapper)
    await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Your prediction"]').setValue('guess'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Your response"]').setValue('answer'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Teach it back"]').setValue('teach'); await wrapper.find('input[value="4"]').setValue(); await wrapper.find('[data-testid="learn-v2-submit"]').trigger('click')
  ...
  it('announces and displays server feedback and next review', async () => {
    const wrapper = await mount(); await startSession(wrapper)
    ;['objective', 'cold_attempt'].forEach(() => {})
    await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Your prediction"]').setValue('guess'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Your response"]').setValue('answer'); await wrapper.find('[data-testid="learn-v2-continue"]').trigger('click'); await wrapper.find('textarea[aria-label="Teach it back"]').setValue('teach'); await wrapper.find('input[value="5"]').setValue(); await wrapper.find('[data-testid="learn-v2-submit"]').trigger('click')
    await vi.waitFor(() => expect(wrapper.text()).toContain('Score: 80%')); expect(wrapper.find('[data-testid="learn-v2-live"]').attributes('aria-live')).toBe('polite'); await wrapper.find('[data-testid="learn-v2-next-review"]').trigger('click'); expect(wrapper.find('[data-testid="learn-v2-next-review-panel"]').exists()).toBe(true)
  })
})
