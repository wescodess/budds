...
export const learningThreadActivityFields = {
  ...
  recoveryFeedback: v.optional(v.object({
    ...
    template: v.literal('provider_unavailable'),
    message: v.string(),
  })),
  feedbackProjection: v.optional(v.object({
    templateVersion: v.literal('learn-adaptive.feedback-templates.v1'),
    taxonomyVersion: v.literal('learn-adaptive.misconception-taxonomy.v1'),
    criterionResults: v.array(v.object({
      key: v.string(),
      label: v.string(),
      awarded: v.boolean(),
      template: v.union(v.literal('criterion_met'), v.literal('criterion_not_met')),
      message: v.string(),
    })),
    misconceptionTags: v.array(v.union(
      v.literal('missing_required_step'),
      v.literal('unsupported_claim'),
      v.literal('confused_concepts'),
      v.literal('incorrect_sequence'),
      v.literal('scope_overgeneralization'),
      v.literal('incomplete_transfer'),
      v.literal('calculation_or_unit_error'),
      v.literal('evidence_mismatch'),
    )),
    misconceptionFeedback: v.array(v.object({
      tag: v.union(
        v.literal('missing_required_step'),
        v.literal('unsupported_claim'),
        v.literal('confused_concepts'),
      ...
      template: v.union(v.literal('evidence_insufficient'), v.literal('response_incomplete')),
      message: v.string(),
    })),
  })),
