...
export const LEARN_V2_CONTRACT: LearnV2Contract = {
  ...
  persistence: {
    ...
    tables: [
      ...
      table('learnObjectives', 'Ordered capability objectives and assessment contracts.', 'ordered_child', [index('userId', 'blueprintRevisionId', 'order')], 'blueprintRevisionId'),
      ...
      table('learnSourceSnapshots', 'Immutable fetched or folder-source revision identity.', 'evidence_identity', [index('userId', 'sourceIdentityId', 'revision'), index('userId', 'learningVoidId', 'status')], 'sourceIdentityId'),
      ...
      table('learnFolderSourceManifestFolders', 'Normalized durable folder traversal checkpoints with captured identity and revision.', 'ordered_child', [index('userId', 'manifestId', 'folderId'), index('userId', 'manifestId', 'stage', 'order')], 'manifestId'),
      table('learnFolderSourceManifestEntries', 'Normalized frozen documents linked to canonical source snapshots.', 'ordered_child', [index('userId', 'manifestId', 'order'), index('userId', 'manifestId', 'documentId')], 'manifestId'),
      ...
      table('masteryAttempts', 'Append-only server-scored attempt ledger.', 'attempt_ledger', [index('userId', 'objectiveId', 'attemptedAt'), index('userId', 'idempotencyKey')], 'objectiveId'),
      table('masteryRecords', 'Derived objective mastery and due-review projection.', 'derived_projection', [index('userId', 'objectiveId'), index('userId', 'nextReviewAt')], 'objectiveId'),
      ...
      table('studyPlanRevisions', 'Immutable feasibility, availability, and schedule decisions.', 'immutable_revision', [index('userId', 'studyPlanId', 'revision'), index('userId', 'learningVoidId', 'status')], 'studyPlanId'),
  ...
  runtime: {
    ...
    pureDecisionModules: ['mastery_projection', 'schedule_feasibility', 'query_redaction', 'fetch_admission'],
