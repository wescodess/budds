...
export const LEARN_V2_CONTRACT: LearnV2Contract = {
  ...
  persistence: {
    ...
    tables: [
      ...
      table('masteryAttempts', 'Append-only server-scored attempt ledger.', 'attempt_ledger', [index('userId', 'blueprintRevisionId'), index('userId', 'blueprintRevisionId', 'kind'), index('userId', 'objectiveId', 'attemptedAt'), index('userId', 'idempotencyKey')], 'objectiveId'),
      table('masteryRecords', 'Derived objective mastery and scheduling-priority projection.', 'derived_projection', [index('userId', 'blueprintRevisionId'), index('userId', 'objectiveId'), index('userId', 'nextReviewAt')], 'objectiveId'),
