...
import { LEARNING_DECISION_MAX_ITEMS, type QuizDecisionItem, type TypedDecisionRequest, type TypedDecisionResult, unavailableDecision } from './contracts'
...
export async function shadowEvaluateQuiz(event: H3Event, items: QuizDecisionItem[]): Promise<TypedDecisionResult> {
  const boundedItems = items.slice(0, LEARNING_DECISION_MAX_ITEMS)
  const serialized = JSON.stringify(boundedItems)
  ...
  const request = { kind: 'quiz_quality' as const, requestId: crypto.randomUUID(), inputDigest: digest, items: boundedItems }
