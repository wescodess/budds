  ...
  handler: async (ctx, args) => {
    ...
    for (const sessionId of reflow.preservedSessionIds) {
      ...
      if (session.status === 'ready' && plan.blueprintRevisionId) {
        const sourceIds = await supportedSourceIds(ctx, userId, session.primaryObjectiveId)
        const inputDigest = await digest({ planRevisionId: String(successorId), sessionId: String(successorSessionId), sessionRevision: 1, objectiveId: String(session.primaryObjectiveId), sourceIds: sourceIds.map(String).sort() })
        const jobId = await ctx.db.insert('learnJobs', { userId, learningVoidId: learningVoid._id, blueprintRevisionId: plan.blueprintRevisionId, studyPlanRevisionId: successorId, studySessionId: successorSessionId, type: 'session_content_generation', status: 'queued', revision: 1, idempotencyKey: `session-content:${successorId}:${successorSessionId}`, requestFingerprint: inputDigest, inputDigest, expectedVoidRevision: learningVoid.revision, expectedBlueprintRecordRevision: plan.blueprintRecordRevision, expectedSessionRevision: 1, attempts: 0, dispatchSupportingSourceSnapshotIds: sourceIds, providerEnabled: process.env.LEARN_V2_SESSION_CONTENT_PROVIDER_ENABLED === 'true', providerModel: process.env.LEARN_V2_SESSION_CONTENT_MODEL?.trim() || undefined, providerPolicyVersion: 'learn-v2.session-content-provider.v1', createdAt: now, updatedAt: now })
        await ctx.scheduler.runAfter(0, internal.learnV2SessionContent.executeSessionContentGeneration, { tokenIdentifier: userId, jobId, expectedRevision: 1 })
      }
    }
