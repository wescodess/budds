  ...
  test('enumerates every V2 collection while the rollout gate is off and redacts retention-sensitive fields', async () => {
    ...
    try {
      ...
      await t.run(async ctx => {
        ...
        const jobId = await ctx.db.insert('learnJobs', { userId: USER_A.tokenIdentifier, learningVoidId: ids.voidId, blueprintRevisionId: ids.revisionId, type: 'private', status: 'queued', revision: 1, idempotencyKey: 'j', requestFingerprint: 'secret', inputDigest: 'secret', candidateDigest: 'secret', providerRequestDigest: `sha256:${'a'.repeat(64)}`, providerRequestBytes: 123, providerRequestVersion: 'request.v1', providerLogPolicyVersion: 'metadata-only-no-payload.v1', providerRetentionPolicyVersion: 'zero_data_retention_requested', providerDeletionPolicyVersion: 'delete_with_v2_job_on_account_deletion', expectedVoidRevision: 1, expectedBlueprintRecordRevision: 1, providerResponseId: 'secret-provider-request', leaseToken: 'secret', leaseExpiresAt: 1, checkpoint: 'secret', terminalReason: 'secret' })
      ...
      expect((await asUser.query(api.dataExport.getUserDataPage, { collection: 'learnJobs', paginationOpts: { cursor: null, numItems: 8 } })).page[0]).not.toHaveProperty('providerRequestDigest')
