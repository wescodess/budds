  ...
  test('enumerates every V2 collection while the rollout gate is off and redacts retention-sensitive fields', async () => {
    ...
    try {
      ...
      await t.run(async ctx => {
        ...
        await ctx.db.insert('learnJobs', { userId: USER_A.tokenIdentifier, learningVoidId: ids.voidId, type: 'private', status: 'queued', revision: 1, idempotencyKey: 'j', leaseExpiresAt: 1, checkpoint: 'secret', terminalReason: 'secret' })
