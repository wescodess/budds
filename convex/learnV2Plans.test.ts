...
describe('Learn V2 study-plan preview and acceptance', () => {
  ...
  test('only marks eligible expired sessions missed and reflows future incomplete sessions without changing mastery', async () => {
    ...
    const successorSessions = await setup.t.run(ctx => ctx.db.query('studySessions').withIndex('by_userId_and_studyPlanRevisionId_and_scheduledStartAt', q => q.eq('userId', identity.tokenIdentifier).eq('studyPlanRevisionId', reflow.successorPlanRevisionId as Id<'studyPlanRevisions'>)).take(10))
    expect(successorSessions).toHaveLength((reflow.changedSessionIds as string[]).length)
    ...
    const expectedPrefetchIds = successorSessions.filter(session => session.status === 'planned').slice(0, 2).map(session => session._id)
    expect(successorJobs.map(job => job.studySessionId).sort()).toEqual(expectedPrefetchIds.sort())
