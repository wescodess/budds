...
export default defineSchema({
  ...
  masteryAttempts: defineTable({ userId: v.string(), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), objectiveId: v.id('learnObjectives'), kind: v.optional(v.literal('calibration')), attemptedAt: v.number(), idempotencyKey: v.string(), requestFingerprint: v.optional(v.string()), serverScorePercent: v.optional(v.number()), usedHint: v.optional(v.boolean()), usedReveal: v.optional(v.boolean()), confidence: v.optional(v.number()), rubricVersion: v.optional(v.string()), result: v.optional(v.string()) }).index('by_userId', ['userId']).index('by_userId_and_blueprintRevisionId', ['userId', 'blueprintRevisionId']).index('by_userId_and_blueprintRevisionId_and_kind', ['userId', 'blueprintRevisionId', 'kind']).index('by_userId_and_objectiveId_and_attemptedAt', ['userId', 'objectiveId', 'attemptedAt']).index('by_userId_and_idempotencyKey', ['userId', 'idempotencyKey']),
  masteryRecords: defineTable({ userId: v.string(), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), objectiveId: v.id('learnObjectives'), state: v.union(v.literal('unseen'), v.literal('learning'), v.literal('guided'), v.literal('independent'), v.literal('retained'), v.literal('needs_review'), v.literal('blocked'), v.literal('provisionally_known')), schedulingPriority: v.optional(v.union(v.literal('remediation'), v.literal('standard'), v.literal('deprioritized'))), recordRevision: v.optional(v.number()), nextReviewAt: v.optional(v.number()), updatedAt: v.optional(v.number()) }).index('by_userId', ['userId']).index('by_userId_and_blueprintRevisionId', ['userId', 'blueprintRevisionId']).index('by_userId_and_objectiveId', ['userId', 'objectiveId']).index('by_userId_and_nextReviewAt', ['userId', 'nextReviewAt']),
  ...
  learnJobs: defineTable({ userId: v.string(), learningVoidId: v.id('learningVoids'), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), type: v.string(), status: v.union(v.literal('queued'), v.literal('leased'), v.literal('running'), v.literal('awaiting_approval'), v.literal('blocked'), v.literal('succeeded'), v.literal('failed'), v.literal('cancelled')), revision: v.number(), idempotencyKey: v.string(), requestFingerprint: v.optional(v.string()), inputDigest: v.optional(v.string()), candidateDigest: v.optional(v.string()), expectedVoidRevision: v.optional(v.number()), expectedBlueprintRecordRevision: v.optional(v.number()), attempts: v.optional(v.number()), providerEnabled: v.optional(v.boolean()), providerModel: v.optional(v.string()), providerPolicyVersion: v.optional(v.string()), providerResponseId: v.optional(v.string()), providerResponseModel: v.optional(v.string()), dispatchSupportingSourceSnapshotIds: v.optional(v.array(v.id('learnSourceSnapshots'))), leaseToken: v.optional(v.string()), leaseExpiresAt: v.optional(v.number()), checkpoint: v.optional(v.string()), terminalReason: v.optional(v.string()), createdAt: v.optional(v.number()), updatedAt: v.optional(v.number()) }).index('by_userId', ['userId']).index('by_userId_and_idempotencyKey', ['userId', 'idempotencyKey']).index('by_userId_and_status_and_leaseExpiresAt', ['userId', 'status', 'leaseExpiresAt']).index('by_status_and_leaseExpiresAt', ['status', 'leaseExpiresAt']).index('by_type_and_status_and_leaseExpiresAt', ['type', 'status', 'leaseExpiresAt']).index('by_userId_and_learningVoidId_and_type', ['userId', 'learningVoidId', 'type']).index('by_userId_and_blueprintRevisionId_and_type_and_status', ['userId', 'blueprintRevisionId', 'type', 'status']).index('by_userId_and_blueprintRevisionId_and_type_and_status_and_terminalReason', ['userId', 'blueprintRevisionId', 'type', 'status', 'terminalReason']),
