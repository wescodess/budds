...
export default defineSchema({
  ...
  masteryRecords: defineTable({ userId: v.string(), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), objectiveId: v.id('learnObjectives'), state: v.union(v.literal('unseen'), v.literal('learning'), v.literal('guided'), v.literal('independent'), v.literal('retained'), v.literal('needs_review'), v.literal('blocked'), v.literal('provisionally_known')), schedulingPriority: v.optional(v.union(v.literal('remediation'), v.literal('standard'), v.literal('deprioritized'))), recordRevision: v.optional(v.number()), firstIndependentPassAt: v.optional(v.number()), firstIndependentLocalDate: v.optional(v.string()), firstIndependentTimezone: v.optional(v.string()), lastAttemptAt: v.optional(v.number()), lastAttemptId: v.optional(v.id('masteryAttempts')), remediationAttemptId: v.optional(v.id('masteryAttempts')), nextReviewAt: v.optional(v.number()), updatedAt: v.optional(v.number()) }).index('by_userId', ['userId']).index('by_userId_and_blueprintRevisionId', ['userId', 'blueprintRevisionId']).index('by_userId_and_blueprintRevisionId_and_objectiveId', ['userId', 'blueprintRevisionId', 'objectiveId']).index('by_userId_and_objectiveId', ['userId', 'objectiveId']).index('by_userId_and_nextReviewAt', ['userId', 'nextReviewAt']),
