...
export default defineSchema({
  users: defineTable({
    ...
    learnAdaptiveExperienceEntitlement: v.optional(v.object({
      enabled: v.boolean(),
      updatedAt: v.number(),
    })),
  ...
  learnActivityCommandReceipts: defineTable(learnActivityCommandReceiptFields)
    ...
    .index('by_redactionStatus_and_resultExpiresAt', ['redactionStatus', 'resultExpiresAt']),

  learnAdaptiveThreadDeletionJobs: defineTable(learnAdaptiveThreadDeletionJobFields)
    .index('by_userId', ['userId'])
    .index('by_userId_and_threadId', ['userId', 'threadId']),
  ...
  masteryAttempts: defineTable({ userId: v.string(), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), objectiveId: v.id('learnObjectives'), studySessionId: v.optional(v.id('studySessions')), sessionContentId: v.optional(v.id('sessionContent')), studyPlanRevisionId: v.optional(v.id('studyPlanRevisions')), kind: v.optional(v.union(v.literal('calibration'), v.literal('independent_application'), v.literal('retained_transfer'))), activityContractVersion: v.optional(v.string()), providerVersion: v.optional(v.string()), attemptedAt: v.number(), attemptLocalDate: v.optional(v.string()), attemptTimezone: v.optional(v.string()), idempotencyKey: v.string(), requestFingerprint: v.optional(v.string()), serverScorePercent: v.optional(v.number()), response: v.optional(v.string()), criterionResultsJson: v.optional(v.string()), misconceptionTagsJson: v.optional(v.string()), feedbackTemplateVersion: v.optional(v.literal('learn-adaptive.feedback-templates.v1')), misconceptionTaxonomyVersion: v.optional(v.literal('learn-adaptive.misconception-taxonomy.v1')), feedbackProjectionJson: v.optional(v.string()), usedHint: v.optional(v.boolean()), usedReveal: v.optional(v.boolean()), confidence: v.optional(v.number()), rubricVersion: v.optional(v.string()), rubricSnapshot: v.optional(v.string()), scorerVersion: v.optional(v.string()), scorerModel: v.optional(v.string()), verifierVersionsJson: v.optional(v.string()), sourceSnapshotIdsJson: v.optional(v.string()), contentRevisionPinsJson: v.optional(v.string()), sessionRevision: v.optional(v.number()), contentRevision: v.optional(v.number()), planRevision: v.optional(v.number()), planRecordRevision: v.optional(v.number()), blueprintRecordRevision: v.optional(v.number()), masteryStateBefore: v.optional(masteryStateValidator), masteryStateAfter: v.optional(masteryStateValidator), masteryTransitionReason: v.optional(masteryTransitionReasonValidator), masteryTransitionVersion: v.optional(v.literal('learn-v2.mastery-transition.v1')), masteryRecordRevision: v.optional(v.number()), result: v.optional(v.string()) }).index('by_userId', ['userId']).index('by_userId_and_blueprintRevisionId', ['userId', 'blueprintRevisionId']).index('by_userId_and_blueprintRevisionId_and_kind', ['userId', 'blueprintRevisionId', 'kind']).index('by_userId_blueprintRevisionId_objectiveId_attemptedAt', ['userId', 'blueprintRevisionId', 'objectiveId', 'attemptedAt']).index('by_userId_and_objectiveId_and_attemptedAt', ['userId', 'objectiveId', 'attemptedAt']).index('by_userId_and_idempotencyKey', ['userId', 'idempotencyKey']),
  // One row is written immediately before a scorer-provider dispatch. This is
  // deliberately separate from jobs: retries retain a single job but provider
  // attempts must be bounded over time.
  learnMasteryScoringRateEvents: defineTable({ userId: v.string(), jobId: v.id('learnJobs'), createdAt: v.number(), expiresAt: v.number() }).index('by_userId', ['userId']).index('by_userId_and_createdAt', ['userId', 'createdAt']).index('by_expiresAt', ['expiresAt']),
  masteryRecords: defineTable({ userId: v.string(), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), objectiveId: v.id('learnObjectives'), scopeKey: v.optional(v.string()), state: masteryStateValidator, schedulingPriority: v.optional(v.union(v.literal('remediation'), v.literal('standard'), v.literal('deprioritized'))), recordRevision: v.optional(v.number()), firstIndependentPassAt: v.optional(v.number()), firstIndependentLocalDate: v.optional(v.string()), firstIndependentTimezone: v.optional(v.string()), lastAttemptAt: v.optional(v.number()), lastAttemptId: v.optional(v.id('masteryAttempts')), remediationAttemptId: v.optional(v.id('masteryAttempts')), nextReviewAt: v.optional(v.number()), updatedAt: v.optional(v.number()) }).index('by_userId', ['userId']).index('by_userId_and_scopeKey', ['userId', 'scopeKey']).index('by_userId_and_blueprintRevisionId', ['userId', 'blueprintRevisionId']).index('by_userId_and_blueprintRevisionId_and_objectiveId', ['userId', 'blueprintRevisionId', 'objectiveId']).index('by_userId_and_objectiveId', ['userId', 'objectiveId']).index('by_userId_and_nextReviewAt', ['userId', 'nextReviewAt']),
  ...
  }).index('by_userId', ['userId']).index('by_userId_and_studySessionId', ['userId', 'studySessionId']).index('by_userId_and_provider_and_externalEventId', ['userId', 'provider', 'externalEventId']).index('by_calendarConnectionId', ['calendarConnectionId']).index('by_calendarConnectionId_and_status', ['calendarConnectionId', 'status']).index('by_connectionId_and_status_and_createLeaseExpiresAt', ['calendarConnectionId', 'status', 'providerCreateLeaseExpiresAt']).index('by_calendarConnectionId_and_status_and_providerCreateSettleAfter', ['calendarConnectionId', 'status', 'providerCreateSettleAfter']),
  ...
  learnJobs: defineTable({ userId: v.string(), learningVoidId: v.id('learningVoids'), blueprintRevisionId: v.optional(v.id('learnBlueprintRevisions')), studyPlanRevisionId: v.optional(v.id('studyPlanRevisions')), studySessionId: v.optional(v.id('studySessions')), adaptiveThreadId: v.optional(v.id('learningThreads')), adaptiveActivityId: v.optional(v.id('learningThreadActivities')), type: v.string(), status: v.union(v.literal('queued'), v.literal('leased'), v.literal('running'), v.literal('awaiting_approval'), v.literal('blocked'), v.literal('succeeded'), v.literal('failed'), v.literal('cancelled')), revision: v.number(), idempotencyKey: v.string(), requestFingerprint: v.optional(v.string()), inputDigest: v.optional(v.string()), candidateDigest: v.optional(v.string()), expectedVoidRevision: v.optional(v.number()), expectedBlueprintRecordRevision: v.optional(v.number()), expectedSessionRevision: v.optional(v.number()), attempts: v.optional(v.number()), providerEnabled: v.optional(v.boolean()), providerVersion: v.optional(v.string()), providerModel: v.optional(v.string()), providerPolicyVersion: v.optional(v.string()), providerRequestVersion: v.optional(v.string()), providerJobVersion: v.optional(v.string()), providerPayloadPolicyVersion: v.optional(v.string()), providerLogPolicyVersion: v.optional(v.string()), providerRetentionPolicyVersion: v.optional(v.string()), providerDeletionPolicyVersion: v.optional(v.string()), providerTimeoutPolicyVersion: v.optional(v.string()), providerQuotaPolicyVersion: v.optional(v.string()), providerPilotManifestVersion: v.optional(v.string()), providerRequestDigest: v.optional(v.string()), providerRequestBytes: v.optional(v.number()), providerMaxRequestBytes: v.optional(v.number()), providerMaxResponseBytes: v.optional(v.number()), providerMaxOutputTokens: v.optional(v.number()), providerTimeoutMs: v.optional(v.number()), providerCostCeilingUsd: v.optional(v.number()), providerResponseId: v.optional(v.string()), providerResponseModel: v.optional(v.string()), dispatchSupportingSourceSnapshotIds: v.optional(v.array(v.id('learnSourceSnapshots'))), leaseToken: v.optional(v.string()), leaseExpiresAt: v.optional(v.number()), checkpoint: v.optional(v.string()), terminalReason: v.optional(v.string()), createdAt: v.optional(v.number()), updatedAt: v.optional(v.number()) }).index('by_userId', ['userId']).index('by_userId_and_idempotencyKey', ['userId', 'idempotencyKey']).index('by_userId_and_status_and_leaseExpiresAt', ['userId', 'status', 'leaseExpiresAt']).index('by_status_and_leaseExpiresAt', ['status', 'leaseExpiresAt']).index('by_type_and_status_and_leaseExpiresAt', ['type', 'status', 'leaseExpiresAt']).index('by_userId_and_learningVoidId_and_type', ['userId', 'learningVoidId', 'type']).index('by_userId_and_blueprintRevisionId_and_type_and_status', ['userId', 'blueprintRevisionId', 'type', 'status']).index('by_userId_and_blueprintId_and_type_and_status_and_terminalReason', ['userId', 'blueprintRevisionId', 'type', 'status', 'terminalReason']).index('by_userId_and_studySessionId_and_type_and_status', ['userId', 'studySessionId', 'type', 'status']),
