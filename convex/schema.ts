...
export default defineSchema({
  ...
  sessionContent: defineTable({ userId: v.string(), studySessionId: v.id('studySessions'), revision: v.number(), status: v.union(v.literal('draft'), v.literal('ready'), v.literal('published'), v.literal('superseded')), assessmentRubricSnapshot: v.optional(v.string()), generatorVersion: v.optional(v.string()), createdAt: v.number(), publishedAt: v.optional(v.number()) }).index('by_userId', ['userId']).index('by_userId_and_studySessionId_and_revision', ['userId', 'studySessionId', 'revision']).index('by_userId_and_status', ['userId', 'status']),
