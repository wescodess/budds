...
export const getUserDataPage = query({
  ...
  handler: async (ctx, args) => {
    ...
    switch (args.collection) {
      ...
      case 'masteryAttempts': { const result = await ctx.db.query('masteryAttempts').withIndex('by_userId', q => q.eq('userId', userId)).paginate(paginationOpts); return { ...result, page: result.page.map(({ idempotencyKey: _idempotencyKey, requestFingerprint: _requestFingerprint, ...row }) => row) } }
