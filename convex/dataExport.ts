...
export const getUserDataPage = query({
  ...
  handler: async (ctx, args) => {
    ...
    switch (args.collection) {
      ...
      case 'masteryAttempts': { const result = await ctx.db.query('masteryAttempts').withIndex('by_userId', q => q.eq('userId', userId)).paginate(paginationOpts); return { ...result, page: result.page.map(({ idempotencyKey: _idempotencyKey, requestFingerprint: _requestFingerprint, response: _response, criterionResultsJson: _criterionResults, rubricSnapshot: _rubricSnapshot, sourceSnapshotIdsJson: _sourcePins, studySessionId: _sessionId, sessionContentId: _contentId, studyPlanRevisionId: _planId, blueprintRevisionId: _blueprintId, objectiveId: _objectiveId, ...row }) => row) } }
