...
export interface LearnPlanSessionSnapshot { id: string; title: string; when: string; durationMinutes: number; kind: 'learning' | 'review' | 'buffer' | 'rest' | 'retained_review'; objectiveId?: string }
export interface LearnPlanSnapshot { accepted: boolean; feasibility: 'feasible' | 'constrained' | 'pending'; headline: string; detail: string; weeklyLoadLabel: string; targetLabel: string; sessions: LearnPlanSessionSnapshot[]; alternatives?: string[]; calendarStatus?: 'not_connected' | 'ready' | 'attention' }
