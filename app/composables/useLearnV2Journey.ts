...
import type { LearnHubSnapshot, LearnMissionSnapshot, LearnOutcomeDraft, LearnPlanSnapshot, LearnReadinessItem, LearnSourceSnapshot, LearnObjectiveSnapshot, LearnObjectiveEdit, LearnScheduleInput } from '~/types/learn-v2-journey'
...
export function useLearnV2Journey(learningVoidId?: MaybeRef<string | null | undefined>) {
  ...
  const sources = computed<LearnSourceSnapshot[]>(() => ((sourceReviewQuery.data.value as any)?.page ?? mission.value?.sources?.items ?? []).map((row: any) => ({ id: String(row._id), title: row.identity?.title ?? row.publicLocator ?? 'Evidence source', origin: row.identity?.origin ?? 'folder_document', publisher: row.identity?.publisherDomain ?? (row.publicLocator ? new URL(row.publicLocator).hostname : undefined), retrievedLabel: row.retrieval?.fetchedAt ? `Retrieved ${dateTime(row.retrieval.fetchedAt)}` : (row.effectiveStatus ?? row.status).replaceAll('_', ' '), coverage: row.effectiveStatus === 'unavailable' ? 'gap' : row.effectiveStatus === 'user_accepted' ? 'strong' : 'partial', lifecycle: row.effectiveStatus ?? row.status, objectives: row.reviewedObjectives?.map((objective: any) => objective.title) ?? [], accessNote: row.rights?.access === 'locator_only' ? 'The original is available by locator, but Budds cannot retain an excerpt.' : undefined, excerpt: row.excerpt })))
  ...
  async function prepareAndAcceptSource(source: LearnSourceSnapshot) { return await run(async () => { const row = mission.value?.sources.items.find((item: any) => String(item._id) === source.id); if (!row) throw new Error('Source is no longer available'); let revision = row.recordRevision ?? 1; if (source.origin === 'folder_document' && row.effectiveStatus !== 'evaluated') { const prepared: any = await prepareFolderSource.mutate({ sourceSnapshotId: row._id, expectedRevision: revision, idempotencyKey: key('prepare-folder-source') }); revision = prepared.recordRevision } return await acceptSourceMutation.mutate({ sourceSnapshotId: row._id, expectedRevision: revision, idempotencyKey: key('accept-source') }) }) }
  ...
  async function addUrlSource(url: string) { const row = mission.value; if (!row?.currentBlueprint || !url.trim()) return; return await run(async () => { const registered: any = await registerCandidate.mutate({ learningVoidId: row.learningVoid._id, blueprintRevisionId: row.currentBlueprint._id, url: url.trim(), expectedVoidRevision: row.learningVoid.revision, expectedBlueprintRecordRevision: row.currentBlueprint.recordRevision, idempotencyKey: key('add-url-source') }); return await fetchAndPrepare.mutate({ sourceSnapshotId: registered.sourceSnapshotId, expectedRevision: registered.recordRevision, idempotencyKey: key('fetch-url-source') }) }) }
  ...
  async function editLearningMap(edit: LearnObjectiveEdit) { const original = mission.value; if (!original?.currentBlueprint || !original.map) return; return await run(async () => {
    const editOrder = edit.objectiveId ? original.map.objectives.find((item: any) => String(item._id) === edit.objectiveId)?.order : undefined
    if (edit.objectiveId && editOrder === undefined) throw new Error('Objective is no longer in this map')
    ...
    const milestoneKeys = new Map(row.map.milestones.map((item: any) => [String(item._id), `milestone-${item.order + 1}`]))
    const objectiveKeys = new Map(row.map.objectives.map((item: any) => [String(item._id), `objective-${item.order + 1}`]))
    const base = row.map.objectives.map((item: any) => ({ key: objectiveKeys.get(String(item._id))!, milestoneKey: milestoneKeys.get(String(item.milestoneId))!, order: item.order, title: item.title, capability: item.capability, estimatedMinutes: item.estimatedMinutes, coverage: item.coverage, gapReason: item.gapReason, sourceSnapshotIds: item.sourceLinks.filter((link: any) => link.coverage !== 'gap').map((link: any) => link.sourceSnapshotId), gapSourceSnapshotIds: item.sourceLinks.filter((link: any) => link.coverage === 'gap').map((link: any) => link.sourceSnapshotId), prerequisiteObjectiveKeys: item.prerequisiteObjectiveIds.map((itemId: any) => objectiveKeys.get(String(itemId))).filter(Boolean), assessmentContract: item.assessmentContract }))
    if (edit.objectiveId) { const target = base.find((item: any) => item.order === editOrder); if (!target) throw new Error('Objective is no longer in this map'); target.title = edit.title.trim(); target.capability = edit.capability.trim() } else { const template = base[base.length - 1]; if (!template || base.length >= 15) throw new Error('This map cannot add another objective'); base.push({ ...template, key: `objective-${base.length + 1}`, order: base.length, title: edit.title.trim(), capability: edit.capability.trim(), prerequisiteObjectiveKeys: template.key ? [template.key] : [] }) }
    return await replaceMap.mutate({ blueprintRevisionId: row.currentBlueprint._id, expectedRecordRevision: row.currentBlueprint.recordRevision, expectedVoidRevision: row.learningVoid.revision, idempotencyKey: key('replace-map'), candidate: { version: 'learn-v2.blueprint-candidate.v1', generatorVersion: 'learn-v2.learner-map-edit.v1', milestones: row.map.milestones.map((item: any) => ({ key: milestoneKeys.get(String(item._id))!, order: item.order, title: item.title, description: item.description })), objectives: base } })
  }) }
  ...
  return { allowed, checkingAccess, hub, mission, snapshot, sources, objectives, readiness, plan, busy, error, generation, generationIsActive, defaultSchedule, planInput, saveOutcomeDraft, createMission, startMapGeneration, editLearningMap, prepareAndAcceptSource, rejectSource, addUrlSource, acceptLearningMap, submitCalibrationAttempt, completeCalibration, createPlanPreview, editPlanPreview, acceptPlan }
}
