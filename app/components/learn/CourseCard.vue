...
const isDeleting = computed(() => props.course.status === 'deleting')
...
<template>
  <NuxtLink
    ...
    :class="[
      ...
      { 'pointer-events-none opacity-75': isDeleting },
    ]"
  ...
  >
    <div class="mb-3 flex items-start justify-between gap-2">
      <h3
        ...
        :class="isFailed || isDeleting ? 'text-stone-400' : 'text-stone-100 group-hover:text-amber-400'"
      >
        {{ course.title }}
      </h3>
      <span
        v-if="isDeleting"
        class="shrink-0 rounded-full bg-stone-500/20 px-2 py-0.5 text-xs font-medium text-stone-400"
